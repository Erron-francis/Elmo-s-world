...
bool is_signed_integer(int num) {
	if (num >= INT_MIN && num <= INT_MAX) {
		return true;  // It is a valid signed integer
		} else {
		return false; // Out of range (though unlikely for `int`)
	}
}
...
void test_power() {
	...
	// Check if the result is within the range of an 8-bit signed integer
	
	if (result > CHAR_MAX || result < -CHAR_MIN) {
		
		printf("Overflow error: result %d exceeds 8-bit signed int range\n", result);
		
		TEST_FAIL_MESSAGE("Overflow error");
